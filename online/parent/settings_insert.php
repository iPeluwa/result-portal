<?php
if(isset($_POST['change_parent_password'])){
	...
	$old_pass =strip_tags(mysqli_real_escape_string($con,$_POST['old_password']));
		...
		$check = "SELECT * FROM `parents` WHERE `id`='$userid' AND `password` = '$old_password'";
		...
		if($num_rows < 1){
			...
			$update = "UPDATE `parents` SET `password`= '$new_password' WHERE `id`= '$userid'";
