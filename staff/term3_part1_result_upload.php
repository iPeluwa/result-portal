...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
        ...
        while (($column = fgetcsv($file, 10000, ",")) !== FALSE) {
          ...
          $select = "SELECT * FROM `term3` WHERE `student_id`='$student_id' AND subject_id='$subject'";
          $query_select = mysqli_query($con,$select);   
         if(mysqli_num_rows($query_select) >= 1){
       
           $sqlInsert = "UPDATE term3 SET note1 = '" . $column[3] . "', project1 = '" . $column[4] . "', test1 = '" . $column[5] . "' WHERE `student_id`='$student_id' AND subject_id='$subject' AND session_id='$current_session_id'";
            ...
            $sqlInsert = "INSERT INTO  term3 (session_id,student_id,fname,lname,teacher_id,class_id,subject_id,note1,project1,test1) VALUES ('$current_session_id','$student_id','$fname','$lname','$userid','$class','$subject','" . $column[3] . "','" . $column[4] . "','" . $column[5] . "')";
            $result = mysqli_query($con, $sqlInsert);
...
<div id="response" class="btn btn-<?php if(!empty($type)) { echo $type . " display-block"; } ?>"><?php if(!empty($message)) { echo $message; } ?></div>
          <h1>Third Term Mid-Term Result</h1>
          <div class="header">
