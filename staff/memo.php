<?php include_once 'header.php'; ?>
  ...
  <button type="button"data-toggle="modal" data-target="#view_unsigned" class="btn btn-primary">Unsigned Memo</button>
          ...
          <h1>Memos</h1>
              ...
              <?php
                $i = 0;
                  $select_memo = "SELECT * FROM `memo` WHERE `sender_id` = '$userid' AND `sender_category` = 'teacher' AND `signature`='Signed' ORDER BY `id` DESC";
                  ...
                  while($row = mysqli_fetch_assoc($query_select_memo)){
                    ...
                    ?>
                    <tr><td><?php echo $i; ?> <td><?php echo $row['date']; ?></td>
              ...
              <?php
                  ...
                  while($row_rec = mysqli_fetch_assoc($query_receive_memo)){
                      $teachers_id = explode("," , $row_rec['teachers_id']);
                      $memo_id = $row_rec['id'];
                      $sent = array();
                  ...
                  $row_sent = mysqli_fetch_assoc($query_show_memo);
                    ...
                    ?>
                     ...
                     <tr><td> <?php echo $I; ?> </td><td><?php echo $sender_name;?> </td> <td><?php echo $row_sent['date']; ?></td>
                      <form method = 'post' action='open_memo.php';>
                        <input type='hidden' value="<?php echo $filename; ?>" name='filename'/>
                   <td> <button class="btn btn-success" type='submit' name="open_memo" >Open </button></td></tr>
                    </form>
                     <?php   }   } ?>
                       </tbody>
                       </table>
