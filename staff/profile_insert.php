...
if(isset($_POST['add_signature'])){
	...
	if($_FILES){
	  ...
	  if($userid){
		...
		$ext = end($array);
		$dir =  "../signature/";
		$new_filename = $userid."_staff_signature.".$ext;
		$new_name = $dir.$new_filename;

		if(( $ext == 'jpg')  || ($ext == 'png')){
			if(move_uploaded_file($tmp_name, $new_name)){
				$insert_signature = "UPDATE `teacher` SET `signature`='$new_filename' WHERE `id`='$userid'";
...
$select_sign = "SELECT * FROM `teacher` WHERE `id`='$userid'";
