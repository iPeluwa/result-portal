<?php include_once 'header.php'; ?>
  ...
  <button type="button"data-toggle="modal" data-target="#view_unsigned" class="btn btn-primary ">Unsigned Memo</button>
          <h1>Memos</h1>
              ...
              <?php
                $i = 0;
                  $select_memo = "SELECT * FROM `memo` WHERE `sender_id` = '$userid' AND `sender_category` = 'accounts' AND `signature`='Signed' ORDER BY `id` DESC";
              ...
              <?php
                  ...
                  while($row_rec = mysqli_fetch_assoc($query_receive_memo)){
                      $accounts_id = explode("," , $row_rec['account_id']);
                      $memo_id = $row_rec['id'];
                      $sent = array();
