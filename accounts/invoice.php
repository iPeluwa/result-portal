      ...
      <div class="col-12">
        ...
        <div class = "col-8 offset-2">
          ...
          <table class="table table-striped table-bordered table-hover">
            ...
            $row = mysqli_fetch_assoc($query_check);//there should be only one row so no while loop
            $invoice_id = $row['id'];
            ...
            foreach($products AS $product){   
              ...
              $product_and_qty = explode("_", $product);
              $product_id = $product_and_qty[0];
              $qty = end($product_and_qty);

              $pick_product = "SELECT * FROM `materials` WHERE `id`='$product_id'";
              $query_pick = mysqli_query($con, $pick_product);
              $material = mysqli_fetch_assoc($query_pick);
              $product_name = $material['material'];
              $product_price = $material['price'];
              if($product_name != ''){
       ...
       <div class="col-lg-5 col-md-5">
        ...
        <table class="table table-striped table-bordered table-hover">
          ...
          <tbody>
            <?php
            $I = 0;
            $receive_invoice = "SELECT * FROM `invoice` WHERE `signature` <> '' ORDER BY `id` DESC";
             ...
             <div class="form-group">
              <div class="form-row">
                <div class="col-md-10 offset-1">
                  ...
                  <input type="number" min='1'class='form-control' required = 'required' name="qty" oninput="validity.valid||(value='')" />
                </div>
