...
if(isset($_POST['change_admin_name'])){
	...
	if($num_rows < 1){
		echo "<script type='text/javascript'>alert('Wrong Password and Email Combination!!!')</script>";
		echo"<script type='text/javascript'> window.location.href = 'settings.php'; </script>";
		exit();
	}else{
		$update = "UPDATE `admin` SET `name`= '$name' WHERE `email`= '$email'";
...
if(isset($_POST['change_admin_email'])){
	...
	if($num_rows_online < 1){
		...
		$update_online = "UPDATE `admin` SET `email`= '$new_email' WHERE `email`= '$email'";
		...
		if($query_update_online){
			...
			if($num_rows < 1){
				...
				$update = "UPDATE `admin` SET `email`= '$new_email' WHERE `id`= '$userid'";
				$query_update = mysqli_query($con,$update);
...
if(isset($_POST['change_admin_number'])){
	...
	if($num_rows < 1){
		...
		$update = "UPDATE `admin` SET `phone`= '$number' WHERE `id`= '$userid'";
		$query_update = mysqli_query($con,$update);
	...
	if($num_rows_online < 1){
		...
		$update_online = "UPDATE `admin` SET `password`= '$new_password' WHERE `email`='$email'";
		$query_update_online = mysqli_query($conn,$update_online);
		...
		if($query_update_online){
			...
			if($num_rows < 1){
				...
				$update = "UPDATE `admin` SET `password`= '$new_password' WHERE `id`= '$userid'";
				$query_update = mysqli_query($con,$update);
