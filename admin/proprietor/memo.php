         ...
         <div class="col-lg-6 col-md-6 ">
            ...
            <table class="table table-striped table-bordered table-hover">
              ...
              <tbody>
                <?php
                $ii = 0;
                $all_memo = "SELECT * FROM `memo` WHERE `signature` = 'Signed' ORDER BY `id` DESC";
                $query_all_memo = mysqli_query($con, $all_memo);
                while($row_all = mysqli_fetch_assoc($query_all_memo)){
                 $ii+= 1;
                 $filename = $row_all['filename'];

                 $senders_id = $row_all['sender_id']; 
                 $senders_category = $row_all['sender_category'];      
                 ...
                 <tr><td><?php echo $ii; ?> </td><td><?php echo $sender_name;?> </td> <td><?php echo $row_all['date']; ?></td>
                   <form method = 'post' action='open_memo.php';>
                    <input type='hidden' value="<?php echo $filename; ?>" name='filename'/>
                    <td> <button class="btn btn-success" action = "open_memo.php" name="open_memo" >Open </button></td></tr>
                  </form>
